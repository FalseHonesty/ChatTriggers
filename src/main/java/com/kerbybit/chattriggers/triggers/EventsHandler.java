        ...
        for (int i = 0; i < tmp_event.size(); i++) {
            try {
                ...
                if (!TMP_e.contains(" ")) {
                    ...
                    TMP_c = TMP_e.substring(0, TMP_e.indexOf(" "));
                    TMP_e = TMP_e.substring(TMP_e.indexOf(" ") + 1, TMP_e.length());
                }
                int TMP_t = -1;
                ...
                try {
                    ...
                    ChatHandler.warn(ChatHandler.color("red", "<time=t> t must be an integer!"));
                }
                if (TMP_t == -1) {
                ...
                try {
                    ...
                    ChatHandler.warn(ChatHandler.color("red", "<pos=p> p must be an integer!"));
                }
                ...
                try {
                    ...
                    ChatHandler.warn(ChatHandler.color("red", "<vol=v> v must be an integer!"));
                }
                ...
                try {
                    ...
                    ChatHandler.warn(ChatHandler.color("red", "<pitch=p> p must be an integer!"));
                }
                ...
                try {
                    ...
                    ChatHandler.warn(ChatHandler.color("red", "<fadein=f> f must be an integer!"));
                }
                ...
                try {
                    ...
                    ChatHandler.warn(ChatHandler.color("red", "<fadeout=f> f must be an integer!"));
                }
                ...
                if (TMP_c.equalsIgnoreCase("SAY") || TMP_c.equalsIgnoreCase("CHAT") || TMP_c.equalsIgnoreCase("KILLFEED") || TMP_c.equalsIgnoreCase("NOTIFY")) {
                    if (TMP_c.equalsIgnoreCase("SAY")) {
                        if (!Minecraft.getMinecraft().isSingleplayer()) {
                            TMP_e = ChatHandler.addFormatting(TMP_e);
                        }
                    } else {
                        TMP_e = ChatHandler.addFormatting(TMP_e);
                    }
                ...
                //non-logic events
                if (TMP_c.equalsIgnoreCase("TRIGGER")) {
                    doTrigger(TMP_e, chatEvent, soundEvent, isAsync);
                }
                ...
                if (TMP_c.equalsIgnoreCase("SAY")) {
                    if (!global.hasWatermark) {
                        global.chatQueue.add(TMP_e);
                    }
                }

                if (TMP_c.equalsIgnoreCase("DEBUG") || TMP_c.equalsIgnoreCase("DO")) {
                    if (global.debug) {
                        ChatHandler.warn(TMP_e);
                    }
                }
                if (TMP_c.equalsIgnoreCase("LOG")) {
                    System.out.println(removeStringReplacements(TMP_e));
                }
                    ...
                    if (i + 1 < tmp_event.size() - 1) {
                        for (int j = i; j < tmp_event.size(); j++) {
                            if (j != tmp_event.size()) {
                                ...
                                //decrease tab
                                if (tmp_event.get(j).toUpperCase().startsWith("END")) {
                                    tabbed_logic--;
                                }
                            }
                ...
                if (TMP_c.equalsIgnoreCase("WAIT")) {
                    ...
                    if (i + 1 < tmp_event.size() - 1) { //check for events after if event
                        for (int j = i; j < tmp_event.size(); j++) {
                            if (j != tmp_event.size()) {
                                if (chatEvent != null) {
                                    tmp_event.set(j, tmp_event.get(j).replace("{msg}", chatEvent.message.getFormattedText()));
                                }
                ...
                if (TMP_c.equalsIgnoreCase("FOR")) {
                    ...
                    if (TMP_e.contains(":") || (TMP_e.contains(" in "))) {
                        String[] tmp_valuefor;
                        if (TMP_e.contains(":")) {
                            tmp_valuefor = TMP_e.split(":", 2);
                        } else {
                            tmp_valuefor = TMP_e.split(" in ", 2);
                        }
                        ...
                        if (tmp_valuefor.length == 2) {
                            ...
                            if (global.debug) {
                                ChatHandler.warn(ChatHandler.color("red", "ERR: for $value in $array -> missing value or array!"));
                            }
                        }
                        ...
                        if (arrayto.size() > 0 && eventsToFor.size() > 0) {
                            if (valwait.equals("")) {
                                ...
                                try {
                                    ...
                                    if (global.debug) {
                                        ChatHandler.warn(ChatHandler.color("red", "ERR: for $value in $array wait $ticks -> expected number for ticks!"));
                                    }
                                }
                            }
                        }
                            ...
                            try {
                                ...
                                if (args.length == 3) {
                                    ...
                                    try {
                                        ...
                                        if (global.debug) {
                                            ChatHandler.warn(ChatHandler.color("red", "ERR: for $i from $n1 to $n2 wait $ticks -> expected number for ticks!"));
                                        }
                                    }
                                }
                            } catch (NumberFormatException e) {
                    ...
                    if (i + 1 < tmp_event.size() - 1) { //check for events after if event
                        for (int j = i; j < tmp_event.size(); j++) {
                            if (j != tmp_event.size()) {
                                ...
                                //move to else
                                if (tmp_event.get(j).toUpperCase().startsWith("ELSE") && tabbed_logic == 1) {
                                    gotoElse = true;
                                }

                                //add to list
                                if (!gotoElse) {
                                    eventsToIf.add(tmp_event.get(j));
                                } else {
                                    eventsToElse.add(tmp_event.get(j));
                                }
                        ...
                        if (TMP_e.trim().startsWith("(") && TMP_e.trim().endsWith(")")) {
                            ...
                            for (String value : checkSplit) {
                                TMP_eSB.append(value).append(" ");
                            }
                    ...
                    if (i + 1 < tmp_event.size() - 1) {
                        for (int j = i; j < tmp_event.size(); j++) {
                            ...
                            if (j != tmp_event.size()) {
                                ...
                                if (j == tmp_event.size() - 1 && eventsToChoose.size() > 0) {
                                    eventsToChoose.add(eventsToChooseSub);
                                }
                        ...
                        int moveEvents = 0;
                        for (List<String> events : eventsToChoose) {
                            moveEvents += events.size();
                        }
                        i += moveEvents - 1;
                    }
                }
                ...
                BugTracker.show(e, "event", tmp_event.get(i));
            }
        }
        return ret;
	}
